# llm/openai.py - OpenAI GPT-4o API-specific implementation
...
def _openai_complete(prompt: str, api_key: str, model_name: str, max_output_tokens: int = 8192) -> str:
    """OpenAI API completion call."""
...
def run_on_paper(paper_text: str, meta: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if not api_key or not api_key.strip():
        raise RuntimeError(
            f"OPENAI_API_KEY not set. Provide it in meta['api_key'] (from frontend) or set OPENAI_API_KEY environment variable. "
    ...
    for idx, ch in enumerate(chunks, 1):
        ...
        try:
            raw2 = _openai_complete(prompt2, api_key, model_name, max_output_tokens=8192)
    ...
    for token in followup_tokens[:15]:
        ...
        try:
            raw_focus = _openai_complete(prompt_focus, api_key, model_name, max_output_tokens=2048)
            ...
            print(f"[OpenAI] Error on followup token {token}: {e}")
            continue
