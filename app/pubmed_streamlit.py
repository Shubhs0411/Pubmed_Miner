...
def extract_disease_from_boolean_query(query: str) -> Optional[str]:
    """Extract disease name from boolean query by looking for [Title] tag."""
    # Look for pattern like "Disease Name[Title]" or "(Disease Name[Title])"
...
def extract_disease_from_natural_language(query: str) -> Optional[str]:
    """Extract disease name from natural language query using pattern matching."""
    query_lower = query.lower()
    
    # Common virus/disease patterns (case-insensitive)
    disease_patterns = [
        r'\b(influenza\s+a|influenza\s+b|influenza\s+c)\b',
        r'\b(dengue\s+virus?|dengue)\b',
        r'\b(chikungunya|chikungunya\s+virus?)\b',
        r'\b(sars-cov-2|sars\s+cov\s*2|covid-19|covid19)\b',
        r'\b(ebola\s+virus?|ebola)\b',
        r'\b(zika\s+virus?|zika)\b',
        r'\b(west\s+nile\s+virus?|west\s+nile)\b',
        r'\b(yellow\s+fever\s+virus?|yellow\s+fever)\b',
        r'\b(japanese\s+encephalitis|japanese\s+encephalitis\s+virus?)\b',
        r'\b(hepatitis\s+a|hepatitis\s+b|hepatitis\s+c)\b',
        r'\b(hiv|human\s+immunodeficiency\s+virus?)\b',
        r'\b(hsv|herpes\s+simplex\s+virus?)\b',
        r'\b(cmv|cytomegalovirus)\b',
        r'\b(rsv|respiratory\s+syncytial\s+virus?)\b',
    ]
    ...
    for pattern in disease_patterns:
        ...
        if match:
            disease_name = match.group(1).strip()
            # Capitalize properly
            if 'influenza' in disease_name.lower():
                if 'a' in disease_name.lower():
                    return "Influenza A"
                elif 'b' in disease_name.lower():
                    return "Influenza B"
                elif 'c' in disease_name.lower():
                    return "Influenza C"
            # For others, capitalize first letter of each word
            return ' '.join(word.capitalize() for word in disease_name.split())
    
    # Fallback: Try to extract first capitalized noun phrase
    # Look for patterns like "dengue virus", "influenza protein", etc.
    words = query.split()
    capitalized_words = []
    for word in words:
...
def main():
                ...
                if not api_key_check and "Custom" not in model_choice:
                    ...
                    try:
                        with st.spinner("Converting query..."):
                            ...
                            st.session_state["query_type"] = "natural"
    ...
    with query_tab2:
        ...
        if manual_query.strip():
            ...
            st.session_state["query_type"] = "boolean"
    
    with query_tab3:
        disease_name = st.text_input(
            "Enter Disease Name",
            placeholder="e.g., Influenza A, Dengue, SARS-CoV-2",
            key="disease_name_input",
            help="Enter just the disease name. The full query will be automatically constructed."
        )
        if disease_name.strip():
            constructed_query = construct_full_query(disease_name.strip())
    ...
    if st.button("🔎 Search PubMed", type="primary", disabled=(not query or not query.strip())):
        if not query.strip():
            ...
            # Extract disease name for filename generation
            disease_name = st.session_state.get("disease_name")
            if not disease_name:
                query_type = st.session_state.get("query_type", "unknown")
                if query_type == "boolean":
                    disease_name = extract_disease_from_boolean_query(query)
                elif query_type == "natural":
                    disease_name = extract_disease_from_natural_language(st.session_state.get("nl_query_input", ""))
                else:
                    # Try both methods
                    disease_name = extract_disease_from_boolean_query(query)
                    if not disease_name:
                        disease_name = extract_disease_from_natural_language(query)
            
            # Store disease name for later use
            if disease_name:
                st.session_state["extracted_disease_name"] = disease_name
